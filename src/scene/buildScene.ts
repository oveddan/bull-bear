...
export const extractProperties = (gltf: ParsableScene): Properties => {
  ...
  const meshOptions = gltfJson.meshes?.map(({ name }, index) => ({
    name: convertName(name, index),
    index
  }));
  ...
  properties.meshes = { options: meshOptions, properties: nodeProperties };
...
export function generateRaycastableChoices(properties: Properties): Choices {
  ...
  properties.meshes?.options.forEach(({ index, name }) => {
    choices.push({
      text: `meshes/${name}`,
      value: `meshes/${index}`
    });
